...
import { defaultCoupons, defaultSettings } from '../data/defaults';
...
const catalog: Product[] = (seedProducts as unknown as Product[]).map((p) => ({
  ...p,
...
export interface StoreState {
  ...
  addComment: (productId: string, comment: Omit<ProductComment, 'id' | 'productId' | 'approved'> & { approved?: boolean }) => void;
  ...
  orders: Order[];
  placeOrder: (order: Omit<Order, 'id' | 'items' | 'status' | 'createdAt' | 'userId'> & { userId?: string | null }) => Order;
    ...
    (set, get) => ({
      ...
      approveComment: (commentId) => {
        ...
        Object.keys(comments).forEach((pid) => {
          comments[pid] = comments[pid].map((c) => (c.id === commentId ? { ...c, approved: true } : c));
        });
        set({ comments });
      ...
      upsertCoupon: (coupon) => {
        ...
        const next = exists ? coupons.map((c) => (c.id === coupon.id ? coupon : c)) : [coupon, ...coupons];
        set({ coupons: next });
      ...
      validateCoupon: (code, subtotal) => {
        ...
        if (!coupon || !coupon.active) return { ok: false, message: 'کد تخفیف نامعتبر یا غیرفعال است' };
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        return {
          ...
          ...stored,
          comments,
          products: stored.products && stored.products.length > 0 ? stored.products : current.products,
...
export const selectApprovedComments = (productId: string) => (s: StoreState): ProductComment[] =>
  (s.comments[productId] || []).filter((c) => c.approved);

export const selectCatalog = (s: StoreState): Product[] => s.products.filter((p) => p.active !== false);
